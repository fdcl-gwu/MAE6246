...
$$
\boxed{x(t)=2\operatorname{Re}\!\left(c e^{\lambda t}v\right).}
$$

The imaginary parts cancel at every time, not only at $t=0$. The two complex modes combine into one real motion.

**Express the result using real sine and cosine functions.** Euler's formula gives
...
Write $c=(a-ib)/2$, where $a,b$ are real. Then the real initial-state expansion is
